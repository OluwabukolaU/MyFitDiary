...
export default function Target() {
  ...
  const [targetPeriod, setTargetPeriod] = useState('weekly'); // Default to weekly
  ...
  const handleFormSubmit = (e) => {
    ...
    const newTarget = {
      ...
      targetDate,
      calorieSource,
      targetPeriod, // Include the target period in the target object
      completed: false,
    };
  ...
  return (
    <div className="flex flex-col items-center p-6 min-h-screen " style={{ backgroundImage: `url(${bg})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
      ...
      <form onSubmit={handleFormSubmit} className="relative bg-white text-gray-800 p-8 rounded-lg shadow-2xl max-w-md w-full mb-10 transform transition-all duration-300 hover:scale-105">
        ...
        <div className="mb-6">
          ...
          {weeklyWeightLoss && (
            <p className="mt-2 text-gray-800">
              To lose {weeklyWeightLoss} kg per {targetPeriod}, you need to burn approximately {caloriesToBurn} calories.
            </p>
          )}
        </div>
        <div className="mb-6">
        ...
        <div className="mb-6">
          <label className="block text-lg font-semibold mb-2 text-purple-600">
            Target Period
          </label>
          <div className="flex">
            <div className="mr-4">
              <input 
                ...
                name="targetPeriod" 
                value="weekly" 
                checked={targetPeriod === 'weekly'} 
                onChange={() => setTargetPeriod('weekly')} 
              />
              <label className="ml-2" htmlFor="weekly">Weekly</label>
            </div>
            <div>
              <input 
                type="radio" 
                ...
                name="targetPeriod" 
                value="monthly" 
                checked={targetPeriod === 'monthly'} 
                onChange={() => setTargetPeriod('monthly')} 
              />
              <label className="ml-2" htmlFor="monthly">Monthly</label>
            </div>
          </div>
        </div>
        <button
      ...
      <div className="bg-white text-gray-800 p-8 rounded-lg shadow-2xl max-w-md w-full">
        ...
        <ul>
          {targets.map((target, index) => (
            <li key={index} className={`mb-6 p-6 border rounded-lg ${target.completed ? 'bg-green-200' : 'bg-white'} shadow-md transform transition-all duration-300 hover:scale-105`}>
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-bold">Weekly Weight Loss Rate: {target.weeklyWeightLoss} kg</p>
